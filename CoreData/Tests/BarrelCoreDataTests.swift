...
class BarrelCoreDataTests: XCTestCase {
    ...
    func testSequenceExtensions() {
        ...
        XCTAssertEqual(Array(fetch.dropLast(2)).last?.name, "Mars")
        XCTAssertNotEqual(Array(fetch.dropLast(3)).last?.name, "Mars")
        XCTAssertEqual(Array(fetch.prefix(4)).last?.name, "Mars")
        XCTAssertNotEqual(Array(fetch.prefix(5)).last?.name, "Mars")
